...
const registratePost = async (body) => {
    try {
        ...
        if (queryResult.length > 0) {
            ...
            const insertQuery = "INSERT INTO usuarios (email, contraseña) VALUES (?, ?)";
            const insertResult = await query(insertQuery, [body.email, body.password]);
...
const nombreUsuarioGet = async (body) => {
    try {
        ...
        const queryResult = await query("SELECT nombre FROM usuarios WHERE email = ?", [body.email]);
        // Comprueba si se encontraron resultados
        if (queryResult.length > 0) {
            // Devuelve el nombre y apellidos del usuario
            return queryResult[0].nombre;
        } else {
